...
model = dict(
    ...
    encoder=dict(
        ...
        refine_layer=dict(
            ...
            scale_range=scale_range,
            restrict_xyz=True,
            unit_xyz=[2 * grid_size, 2 * grid_size, 2 * grid_size],
            refine_manual=[0, 1, 2],
            phi_activation=phi_activation,
            semantics=semantics,
            semantic_dim=semantic_dim,
    ...
    head=dict(
        ...
        empty_args=dict(
            _delete_=True,
            mean=[(pc_range[0] + pc_range[3]) / 2.0, (pc_range[1] + pc_range[4]) / 2.0, (pc_range[2] + pc_range[5]) / 2.0],
            scale=[pc_range[3] - pc_range[0], pc_range[4] - pc_range[1], pc_range[5] - pc_range[2]],
        ),
